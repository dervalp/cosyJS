 ...
 * @returns {Object}
 */
var Page = function(_c, config, componentsConfig, placeholder, prefix) {
    this.config = config || { };
    ...
    _.each(componentsConfig, function(component) {
        try {
            var Klass = _c.components[component.type],
                data = _.extend(component, component.data),
                model = new Klass.model(data),
...
Page.prototype.render = function(cb)  {
    ...
    pageTemplate = _.template(self.layout.template())({ content: content, conf: self.buildDynamic() });
